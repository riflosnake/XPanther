...
class XPanther:
    ...
    def capture(self, target):
        ...
        element_regex, attr_regex, closing_tag_regex = self.__regex_distributor()
        ...
        if type(target) == str and re.match(element_regex, target):
            ...
            if occurrences:
                for index_of_element in indexes_of_element:
                    ...
                    self.__print("\033[96m {}\033[00m".format(f'with {type_of_element}-index -> ({row_index + 1} / {len(self.__keys)}) \u2B07'))
