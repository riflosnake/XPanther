...
class XPantherGUI:
    def __init__(self):
        ...
        self.window.iconphoto(True, tk.PhotoImage(file=self.resource_path('xpanther_logo.png')))
    ...
    def launch(self):
        ...
        if self.url:
            if not (result := XPantherIDE(self.url, advanced=self.advanced_var.get(), GUI=True, language=self.selected_option.get()).start()):
                result = 'No selection or something went wrong...'
            else:
                result = ' '.join(result)
    ...
    def show_results(self, info):
        ...
        self.result_window.iconphoto(True, tk.PhotoImage(file=self.resource_path('xpanther_logo.png')))
